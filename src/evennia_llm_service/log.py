...
"""Logging shim for evennia-llm-service.

Every line the library emits goes to its own ``llm_service.log``, co-located
with Evennia's other logs under ``settings.LOG_DIR``, so diagnosing a provider
outage or a missing prompt means reading one file rather than picking it out of
the main server log.
...
logs use — so a service line and a ``server.log`` line can be read against each
...
bootstrapped — ``llm_service_log`` is a silent no-op. The import is lazy and an
ImportError is swallowed; the library deliberately does not fall back to stderr
or a local file.

The pattern is ``evennia-shards``' ``log.py``, by way of ``evennia-ai-memory``.
Internal to the library, not part of the consumer-facing API.
"""

import traceback

_LOG_FILENAME = "llm_service.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def llm_service_log(message: str, level: str = "INFO", trace: bool = False) -> None:
    """Emit one line to ``llm_service.log``.
